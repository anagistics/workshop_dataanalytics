...
.tmp <- mtcars2 %>% select(ids, mpg) %>% mutate(Verbrauch = mpg2lkm(mpg)) %>% top_n(-12)

.tmp %>% ggplot(aes(x = ids, y = Verbrauch)) + geom_col(fill = "darkblue") +
...
```{r}
.tmp %>% ggplot(aes(x = reorder(ids, Verbrauch), y = Verbrauch)) + 
