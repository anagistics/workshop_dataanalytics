---
title: "Datenanalyse und Data Science"
...
* Datenschutz
* Datenquellen lesen
* Daten verstehen: Struktur, Validierung
...
.pull-right[
* Voraussetzungen
  * Installation von [`OpenRefine`](https://openrefine.org)
  * Anmeldung `RStudio Cloud`
  
* `r emo::ji("worker")` Validierung 
* `r emo::ji("worker")` `r emo::ji("worker")` Hypothesenbildung
* `r emo::ji("worker")` `r emo::ji("worker")` Visualisierung
* `r emo::ji("worker")` `r emo::ji("worker")` Reproduzierbarkeit der Datenanalyse 

* `r emo::ji("worker")` `r emo::ji("worker")` `r emo::ji("microscope")`: Praxis Datenanalyse
* `r emo::ji("worker")` `r emo::ji("worker")` `r emo::ji("microscope")`: Praxis Data Science
]
...
Für alle oben genannten Datenquellen gilt: Die meisten davon werden überall in Unternehmen genutzt und sind nicht spezifisch für die Finanzbranche.
...
---
# Besonderheiten von Datenquellen in Unternehmen der Finanzbranche

Banken und Versicherungen vertreiben oftmals sehr langlebige Produkte:
...
* Die gespeicherten Daten sind besonders schützenswert, da Finanz- und/oder Gesundheitsdaten.
...
* **Ignorieren**: Wie gehen fehlende Werte z.B. in eine Mittelwertberechnung ein?
* **Ersetzen**: Durch was? 
.small[
```{r}
...
.center[![:scale 90%](./img/dtreecharts.png)]
...
Ein heute sehr verbreitetes Versionskontrollsystem ist [git](https://www.git-scm.com/). Ein typischer Arbeitsfluss mit *git* in einem Team sieht so aus:

.center[
  ![:scale 80%](./img/git-mainline.png)
]
.center[Quelle: [https://martinfowler.com/articles/branching-patterns.html](https://martinfowler.com/articles/branching-patterns.html)]
...
* ob sich Dateien inhaltlich verändert haben oder
* sicherstellen, dass sich Dateien  *nicht* verändert haben.
...
---
# Vorgehen bei der Modellierung: Baseline-Modell

* Um den praktischen Nutzen eines neuen Modells einzuschätzen, überlegt man sich ein simples pragmatisches Konkurrenzmodell

`r emo::ji("chef")` Dieses Baseline-Modell ist von der Praxis geleitet:
  * *Das Wetter von morgen ist so wie das heutige*
  * *Die monatliche Umsatzentwicklung entspricht der des Vorjahres*
  * *Kunden über 70 nutzen kein Online-Portal*
  * Ein einfaches Modell (bspw. ein lineares Regressionsmodell, s. unten) kann ein Baseline-Modell für komplexere Modelle sein
  * Durchschnitte oder Mediane können auch gute Baseline-Schätzer sein

* Ein neues analytisches Modell sollte deutlich bessere Ergebnisse bringen, um den investierten Aufwand zu rechtfertigen

---
# Vorgehen bei der Modellierung: Gütekriterien Regression

* Präzisierung der Fragestellung, so dass die *Art des Modells* klar ist: Regression, Klassifikation, Prognose, Clusterung...
* Festlegen von *Gütekriterien* in Abhängigkeit von der Modellart:

* **Regression**: 
  * Tatsächlicher Wert der abhängigen Variable $y$
  * Durch das Modell geschätzter Wert $\hat{y}$
  * Minimierung einer *loss function*, bspw. 
      * Mittlerer Quadratischer Fehler (engl. MSE) $\min \sum_{i=1}^n (y_i - \hat{y}_i)^2$ oder 
      * Mittlerer Absoluter Fehler (engl. MAE) $\min \sum_{i=1}^n |y_i - \hat{y}_i|$.

---
# Vorgehen bei der Modellierung: Gütekriterien Klassifikation

* **Klassifikation**: 
  * Basis ist die *Konfusionsmatrix* (confusion matrix):

```{r img-conf, echo=FALSE, fig.align = 'center', out.width = "80%", fig.cap = "Quelle: [3]"}

knitr::include_graphics("./img/konfusion.png")
```

  * Darauf aufbauend werden verschiedene Gütekriterien definiert:
    
.pull-left[
$\text{Spezifizität} = \frac{TN}{TN + FP}$
...
$\text{Richtigkeit} = \frac{TP + TN}{TP + TN + FP + FN}$
]

* Darüberhinaus gibt es noch viele weitere Kriterien, s. Literatur oder [Wikipedia](https://en.wikipedia.org/wiki/Sensitivity_and_specificity)

---
# Vorgehen bei der Modellierung: Beispiel Klassifikationsbewertung

.center[
![:scale 60%](./img/rki-test.png)
]
.center[Quelle: RKI]
]
  
---
# Vorgehen bei der Modellierung (3)

* Festlegen einer Trainings- und Teststrategie:
  * Ziel: Das Modell soll *generalisieren*, d.h. gute Ergebnisse bei der Anwendung auf neuen Daten. Vermeiden von *overfitting* .
    
```{r img-mofi, echo=FALSE, fig.align = 'center', out.width = "60%", fig.cap = "Quelle: [2]"}

knitr::include_graphics("./img/model-fitting.png")
```

---
# Vorgehen bei der Modellierung (4)
  
* Trennung der Daten in einen *Trainingsdatensatz* (größer) und einen *Testdatensatz* (kleiner).
  * Je nach Strategie und Modellart auch noch *Validierungsdatensatz*
  * Man unterscheidet verschiedene Strategien: *zufällige Aufteilung (Split)*, *Kreuzvalidierung*
* Auswahl eines *Lernalgorithmus* passend zur Modellart
* Durchführung des Trainings (*Lernens*, vergleichsweise langwierig):
  * Der Algorithmus optimiert die Parameter des Modells, so dass die Güte maximal wird
      * Zunächst wird dies auf dem Trainingsdatensatz durchgeführt (*in sample* Güte) 
      * und dann auf dem Testdatensatz (*out of sample* Güte). 
* Das Ergebnis des Trainings wird als Datenstruktur gespeichert 
      * Bspw. speichert man die Information *Lineares Regressionsmodell* und die Werte für $\beta_i$
* und zu einem späteren Zeitpunkt wieder geladen und auf neuen Daten angewendet.


---
# Modelltypen: Lineare Regression - Grundlagen

* Regressionsmodell
* **Lineares Modell**: Es wird ein linearer Zusammenhang zwischen den unabhängigen und der abhängigen Variable unterstellt:

.center[
<iframe width="560" height="315" src="https://www.youtube.com/embed/pfAnVIHYE4A" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
]

---
# Modelltypen: Lineare Regression - Berechnung

.center[
  $\min_{\beta_0, \beta_1,\ldots,\beta_n} \|y-\hat{y}\|_2 \text{ mit } \hat{y}=\beta_0 + \sum_{i=1}^n \beta_i\cdot x_i$
]

.pull-left[
Die $\beta_i$ lassen sich relativ leicht berechnen.
Bei nur 1 unabhängigen Variablen, $N$ = Anzahl Beobachtungen:

$\hat{\beta}_1 = \frac{\sum_{j=1}^N(x_j - \bar{x})(y_i-\bar{y})}{\sum_{j=1}^N(x_j - \bar{x})^2}$

$\hat{\beta}_0 = \hat{y}-\hat{\beta}_1 \bar{x}$

mit $\bar{x}=\frac{1}{N}\sum_{j=1}^N x_j$ 

und $\bar{y}=\frac{1}{N}\sum_{j=1}^N y_j$
]

.pull-right[
![:scale 100%](./img/lineare_regression.png)
.center[Quelle: [2]
]
]

---
# Modelltypen: Entscheidungsbäume (1)

* Klassifikationsmodell (häufigste Anwendung, auch Regression möglich)

.pull-left[
* Algorithmus ermittelt eine Baumstruktur von Fallunterscheidungen auf der Basis von Merkmalsausprägungen mit dem Ziel, möglichst homogene *Blätter* zu erhalten.
  * Homogen bezieht sich hierbei auf die Verteilung der Ausprägungen der abhängigen Variable in allen Datensätzen, die zum Blatt gehören.
]

.pull-right[
![:scale 100%](./img/dtree_1.png)
.center[Erste Verzweigung eines Entscheidungsbaums, Quelle: [1]
]
]

---
# Modelltypen: Entscheidungsbäume (2)

* Weitere Verzweigungen der jeweils aktuellen Blätter des Baums, bis eine Abbruchbedingung erfüllt ist:

.center[
![:scale 65%](./img/dtree_example.png)
...
---
# Vergleich lineare Modelle und Entscheidungsbaum

* Beide Verfahrensfamilien haben ihre spezifischen Stärken und Grenzen

```{r img-comp-lm-dtree, echo=FALSE, fig.align = 'center', out.width = "90%", fig.cap = "Quelle: [1]"}
knitr::include_graphics("./img/comp-linear-dtree.png")
```  


---
# Modelltypen: Logistische Regression - Ausgangsproblem

* Abhängige Variable $y\in{0,1}$ (2 Klassen)
* Was passiert, wenn die Klassen mit *linearer Regression* vorhergesagt werden sollen?
* **Beispiel**: Vorhersage von Kreditausfallwahrscheinlichkeit (0/1) als Funktion des Kontostands.

.pull-left[
```{r img-log-reg-motv, echo=FALSE, fig.align = 'center', out.width = "140%", fig.cap = "Quelle: [2]"}

knitr::include_graphics("./img/log-reg-motiv.png")
```
]

.pull-right[
* Problem: Vorhersage liefert tlw. negative Werte
]

---
# Modelltypen: Logistische Regression - Logistische Funktion


.pull-left[
 Zunächst Transformation der unabhängigen Variablen mit der *logistischen Funktion* $p(x)$: 

$p(x) = \frac{e^{\beta_0+\beta_1\cdot x_1+\ldots+\beta_n\cdot x_n}}{1 + e^{\beta_0+\beta_1\cdot x_1+\ldots+\beta_n\cdot x_n}}$
...
.pull-right[
```{r img-log-reg, echo=FALSE, fig.align = 'center', out.width = "150%", fig.cap = "Quelle: [2]"}
...
---
# Modelltypen: Logistische Regression - Klassifikation

* Mit Hilfe der logistischen Funktion $p(x)$ erhält man:
.center[
$\log\left(\frac{p(x)}{1-p(x)}\right) = \beta_0 + \sum_{i=1}^n \beta_i\cdot x_i$.
]

* Der Term $\log\left(\frac{p(x)}{1-p(x)}\right)$ heißt *log odds*. Der Quotient $\frac{p(x)}{1-p(x)}$, die *odds*, gibt eine *Chance* an, wie sie bei Wetten verwendet wird.
* Die *log odds* lassen sich leicht aus den Daten berechnen und die mathematische Struktur entspricht damit wieder einer linearen Regression: 
&rarr; optimale $\beta_i$ berechnen

&rarr; **Klassifikation** anhand eines Schwellwerts $\theta$, so dass 
.center[ $p(x) > \theta \Rightarrow \text{Klasse von x = 1}$
]

---
# Modelltypen: Clusterung - Grundlagen

* Unüberwachtes Lernen: Es gibt keine explizite abhängige Variable.
* Idee: 
  * Variablen (Merkmale) entsprechen Dimensionen eines mehrdimensionalen Raums
  * Die Merkmalsausprägungen einer Instanz definieren damit einen Punkt in diesem Raum
  * Räumlich benachbarte Punkte entsprechen ähnlichen Instanzen
  * Es werden Gruppen (Cluster) von ähnlichen Instanzen gebildet

```{r img-cluster, echo=FALSE, fig.align = 'center', out.width = "60%", fig.cap = "Quelle: [²]"}

knitr::include_graphics("./img/cluster.png")
```

---
# Modelltypen: Clusterung - $k$-Means

* $k$-Means-Algorithmus
```{r img-kmeans, echo=FALSE, fig.align = 'center', out.width = "80%", fig.cap = "Quelle: [2]"}

knitr::include_graphics("./img/cluster-algo.png")
```

* Benötigt wird also ein *Distanzmaß*, eine *Metrik*
* Die Merksmalsausprägungen müssen demnach mindestens intervallskalierte Zahlen sein

---
# Modelltypen: Clusterung - Beispiel

```{r img-cluster-iter, echo=FALSE, fig.align = 'center', out.width = "50%", fig.cap = "Quelle: [2]"}

knitr::include_graphics("./img/cluster-iter.png")
```

* Die berechneten Clusterschwerpunkte sind berechnete Punkte, keine Instanzen
* Die optimale Clusteranzahl ist unklar und nicht einfach bestimmbar
* Das Ergebnis des Verfahrens sind die Cluster, ohne Charakterisierung
...
[Video zu (multipler) linearer Regression](https://youtu.be/ef0efKL1Els)

[Voraussetzungen für lineare Regression](https://youtu.be/MrAPaLLbZQE)
...
**[4]** YouTube-Kanal [Statistik am PC](https://www.youtube.com/c/StatistikamPC_BjoernWalther)
