---
title: "Übungen für Kurse Datenanalyse und Data Science"
date: "Mai - Juni 2021"
output:
...
Die folgende Aufgabe ist als Gruppenübung angelegt. Jede Gruppe/jeder Breakout-Raum, hat zur Aufgabe, ein *Gestaltprinzip* zu erklären: Die sog. *Gestaltprinzipien* sind ein Satz von grundlegenden Prinzipien für gute Visualisierungen. 
  ...
  * Führen Sie Ihre Ergebnisse in einer kurzen Präsentation zusammen (Dauer: ca. 8 Minuten)
...
# Aufgabe (**Gruppe**): Praktische Durchführung von Datenanalysen
...
1. Verwenden Sie das Paket `tidyverse` für die explorative Analyse der Daten und stellen Sie die Ergebnisse Ihrer Analyse graphisch dar. Halten Sie auch diese Ergebnisse in Ihrer Präsentation fest und denken Sie an die Prinzipien für gute Visualisierung. Interessante Fragen könnten z.B. sein:
...
Ein Telekommunikationsanbieter hat für Sie als Datenanalyst eine für Serviceanbieter recht typische Aufgabenstellung: Es geht um die Feststellung der Ursachen für Kundenabwanderungen. Interessant sind Fragestellungen, ob die beobachteten Abwanderungen sich räumlich, zeitlich oder bezüglich einzelner Serviceleistungen konzentrieren. Als Datenanalyst sollen Sie dabei unterstützen, Ursachen für die Abwanderung zu identifizieren. Aus den Vorgesprächen haben Sie schon folgendes erfahren:

* Die Leiterin der DSL-Sparte sieht sich Vorwürfen ausgesetzt, die besagen, dass  Kunden wg. Unzufriedenheit mit dem DSL-Service abwandern.
* Manche Experten glauben, dass die Kunden tendenziell nach 24 Monaten abwandern.
* Marketing geht davon aus, dass es in einzelnen Regionen starke Wettbewerber gibt, die Kunden verstärkt abwerben.

Ihre Aufgabe ist es, zunächst diesen drei Vermutungen nachzugehen. Formulieren Sie dazu zunächst Hypothesen, die Sie anschließend anhand einer Datenanalyse untersuchen.
...
* Die `dplyr`-Funktionen `group_by`, `summarise` und `filter` sind sicherlich hilfreich. Ebenso die Funktionen `forcats::fct_lump_n` und `ggplot2::cut_interval`.
* Weil `R` für Sie wahrscheinlich neu ist, können Sie zunächst versuchen, Ihre Auswertung in einem Pseudocode oder in SQL zu formulieren und dann bei Bedarf auch Beratung anfordern.
